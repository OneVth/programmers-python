...
def solution_v1(str_list: list[str], ex: str) -> str:
    """
    [Approach] generator expression으로 ex 미포함 문자열 필터링 후 join
    [Time] O(n * m) - n: 리스트 길이, m: 문자열 평균 길이 (in 연산)
    [Space] O(결과 문자열 길이)
    """
